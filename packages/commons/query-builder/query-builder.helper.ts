...
import { QueryBuilderHelper, QueryOptions, QueryCondition, PaginatedQueryResult } from './query-builder.interface'
...
export class QueryBuilderHelperImpl<T extends ObjectLiteral> implements QueryBuilderHelper<T> {
  ...
  buildQuery(options: QueryOptions, debug = false): SelectQueryBuilder<T> {
    ...
    if (joins && joins.length > 0) {
      for (const join of joins) {
        ...
        const relationPath = join.property.includes('.') ? join.property : `${alias}.${join.property}`
        ...
        switch (joinMethod) {
          case 'leftJoin':
            ...
            if (join.condition) {
              queryBuilder = queryBuilder.leftJoin(relationPath, join.alias, join.condition)
            } else {
          ...
          case 'innerJoin':
            ...
            // 如果有自定义条件则使用，否则让TypeORM自动处理关联
            if (join.condition) {
              queryBuilder = queryBuilder.innerJoin(relationPath, join.alias, join.condition)
            } else {
          ...
          case 'leftJoinAndSelect':
            ...
            // 如果有自定义条件则使用，否则让TypeORM自动处理关联
            if (join.condition) {
              queryBuilder = queryBuilder.leftJoinAndSelect(relationPath, join.alias, join.condition)
            } else {
          ...
          case 'innerJoinAndSelect':
            ...
            // 如果有自定义条件则使用，否则让TypeORM自动处理关联
            if (join.condition) {
              queryBuilder = queryBuilder.innerJoinAndSelect(relationPath, join.alias, join.condition)
            } else {
              queryBuilder = queryBuilder.innerJoinAndSelect(relationPath, join.alias)
            }
            break
        }
      }
    ...
    if (orderBy && orderBy.length > 0) {
      for (const order of orderBy) {
        ...
        // 否则添加主表别名前缀
        const orderField = order.field.includes('.') ? order.field : `${alias}.${order.field}`
  ...
  async findPaginated(
    ...
    const countExpression = hasJoins ? `COUNT(DISTINCT ${alias}.id)` : `COUNT(${alias}.id)`
