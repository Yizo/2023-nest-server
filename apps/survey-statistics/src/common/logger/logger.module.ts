      ...
      imports: [
        WinstonModule.forRootAsync({
          ...
          useFactory: (configService: ConfigService) => {
            ...
            // 自定义时间戳格式器（北京时间）
            const beijingTimeFormat = winston.format((info: Logform.TransformableInfo) => {
              const date = info.timestamp ? new Date(info.timestamp as string) : new Date()
            ...
            const fileFormat = winston.format.combine(beijingTimeFormat(), winston.format.json())
