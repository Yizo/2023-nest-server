 ...
 */
@Injectable()
export class QueryBuilderHelperImpl<T extends ObjectLiteral>
  implements QueryBuilderHelper<T>
{
  ...
  buildQuery(options: QueryOptions, debug = false): SelectQueryBuilder<T> {
    const { joins, conditions, orderBy, groupBy, having, withDeleted, select } =
      options;
    // 获取实体表名作为查询别名
   ...
   * @private
   */
  private sortConditionsByIndexPriority(
    conditions: QueryCondition[],
  ): QueryCondition[] {
    const priorityMap = {
      eq: 1, // 等值查询，索引利用最佳
  ...
  private applySingleCondition(
    ...
    const paramName = `${cleanField}_${Math.random()
      .toString(36)
      .slice(2, 11)}`;
  ...
  async count(options: QueryOptions): Promise<number> {
    ...
    const countQuery = queryBuilder
      .clone()
      .offset(undefined) // 移除OFFSET
      .limit(undefined) // 移除LIMIT
      .orderBy(undefined) // 移除ORDER BY
      .select(countExpression, 'count');
